...
func to_serve_state(
	serving_position: Vector3, service_hit_angle: float, hit_direction: int
) -> void:
	...
	var serve_state = PlayerServeState.new(player, service_hit_angle, hit_direction)
	player.state = serve_state
...
func to_point_ended_state():
	...
	player.state = point_ended_state
